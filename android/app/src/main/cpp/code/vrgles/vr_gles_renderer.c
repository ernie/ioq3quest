/*
 * vr_gles_renderer.c - GL ES VR rendering
 *
 * Contains the main VR rendering loop for GL ES.
 * Moved from vr/vr_renderer.c
 */

#include "../vrcommon/vr_base.h"
#include "vr_gles_renderer.h"

// Include xr_linear.h for XR math functions (pointer-based API)
...
// APIENTRY is not defined on Android, use GL_APIENTRY instead
#ifndef APIENTRY
...
// Static variable for loading frame state
...
void VR_DrawFrame( engine_t* engine ) {
    ...
    // Store view poses for EndFrame
    XrPosef invViewTransform[2];
    for (int eye = 0; eye < ovrMaxNumEyes; eye++) {
        invViewTransform[eye] = projections[eye].pose;
    }

    // Update HMD position/views (matching q3vr pattern - IN_VRUpdateHMD handles
    // FOV calculation, center pose lerping, eyePose population, etc.)
    XrFovf fov = {};
    IN_VRUpdateHMD(projections, projectionCountOutput, &fov);
    ...
    if (useOverlay) {
        ...
        re.SetScreenOverlayBuffer(
            ...
            0, 0, 0);  // mainSceneReadBuffer/Width/Height not used by GL ES
    } else {
        // Clear overlay buffer info when not using overlay
        re.SetScreenOverlayBuffer(0, 0, 0, 0, 0, 0);
    }
    ...
    re.SetVRHeadsetParms(projectionMatrix.M[0], monoVRMatrix.M[0], glFramebuffer,
                         NULL, NULL, vr.fov_x, 0.0f);
...
// loadingFrameDisplayTime tracks the display time for the loading frame
static XrTime loadingFrameDisplayTime = 0;

static int VR_PrepareLoadingFrame( engine_t* engine )
{
...
static int VR_SubmitLoadingFrame_Internal( engine_t* engine )
{
...
/*
==================
VR_Renderer_SubmitLoadingFrame

Called during loading states (CA_LOADING/CA_PRIMED) from SCR_UpdateScreen.
For GLES renderer, uses the two-phase pattern internally (submit current frame,
...
int VR_Renderer_SubmitLoadingFrame( engine_t* engine )
{
	...
	if (!VR_SubmitLoadingFrame_Internal(engine)) {
		return qfalse;
	}

	// Prepare next frame (so SCR_UpdateScreen has a valid framebuffer to draw to)
	VR_PrepareLoadingFrame(engine);
