// ui_update.c -- engine self-update menu (Android APK variant)
...
static void UpdateMenu_RefreshStatus( void ) {
	...
	case 3: // DOWNLOADING
		...
		break;
	case 4: // READY
		Q_strncpyz( statusBuf, "Update downloaded. Tap Install to update.", sizeof( statusBuf ) );
		break;
	case 5: // ERROR
...
static void UI_UpdateMenu_Event( void *ptr, int event ) {
	...
	case ID_CANCEL:
		...
		break;
	case ID_INSTALL:
		trap_Cmd_ExecuteText( EXEC_NOW, "updateinstall\n" );
		break;
	case ID_BACK:
		UI_PopMenu();
		break;
...
static void UI_UpdateMenu_Init( void ) {
	...
	s_update.install.generic.type		= MTYPE_PTEXT;
	s_update.install.generic.flags		= QMF_CENTER_JUSTIFY | QMF_PULSEIFFOCUS;
	s_update.install.generic.x			= 320;
	s_update.install.generic.y			= y;
	s_update.install.generic.id			= ID_INSTALL;
	s_update.install.generic.callback	= UI_UpdateMenu_Event;
	s_update.install.string				= "INSTALL UPDATE";
	s_update.install.color				= color_red;
	s_update.install.style				= style;
	...
	} else if ( lastBuiltState == 4 ) {
		Menu_AddItem( &s_update.menu, &s_update.install );
	}
