...
// Input attachment for reading scene color from subpass 0 (tile-local)
...
// Specialization constants (same IDs as gamma.frag and blend.frag)
...
void main() {
	// Read scene color from input attachment (tile-local, no DRAM access)
	vec3 base = subpassLoad(sceneColor).rgb;
