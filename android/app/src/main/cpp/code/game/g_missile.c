...
gentity_t *fire_nail( gentity_t *self, vec3_t start, vec3_t forward, vec3_t right, vec3_t up ) {
	...
	bolt->parent = self;
	{
		...
		r = random() * M_PI * 2.0f;
		u = sin(r) * crandom() * gp->weapons[WP_NAILGUN].spread * 16;
		r = cos(r) * crandom() * gp->weapons[WP_NAILGUN].spread * 16;
		...
		if ( gp->weapons[WP_NAILGUN].speed > 0 ) {
			scale = gp->weapons[WP_NAILGUN].speed;
		} else {
...
gentity_t *fire_prox( gentity_t *self, vec3_t start, vec3_t dir ) {
	...
	bolt->parent = self;
	{
		const modeConfig_t *gp = Mode_GetConfig( g_mode.integer );
		bolt->damage = 0;
		bolt->splashDamage = gp->weapons[WP_PROX_LAUNCHER].splashDamage;
		bolt->splashRadius = gp->weapons[WP_PROX_LAUNCHER].splashRadius;
	}
